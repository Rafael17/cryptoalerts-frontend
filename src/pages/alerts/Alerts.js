...
class PriceAlerts extends Component {
	...
	updateAlerts = () => {
		const userId = ls.get('userData')._id;
		fetch(`/api/priceAlerts/?userId=${userId}&status=`, { headers: { 'Content-Type': 'application/json' } })
			.then(response => response.json())
			.then((result) => {
				this.setState({ priceAlerts: result });
			})
	}

	componentDidMount() {
		this.updateAlerts();
		this.getExchangePairsList();
	}

	getExchangePairsList = () => {
		fetch(`http://localhost:3010/prices`, { headers: { 'Content-Type': 'application/json' } })
			.then(response => response.json())
			.then((result) => {
				const parsedPairs = result.map(({ name, tickers }) => {
					const pairs = [];
					for (let key in tickers) {
						const value = name + " - " + key;
						const price = tickers[key];
				...
				this.setState({ pairs: [].concat.apply([], parsedPairs) });
			})
	}
